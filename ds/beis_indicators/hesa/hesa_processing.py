...
def hesa_parser(url, out_name, skip=16, encoding="utf-8"):
    ...
    if f"{out_name}.txt" not in os.listdir(f"{project_dir}/data/raw/hesa/"):
        ...
        # Read it.
        my_csv = pd.read_csv(
            f"{project_dir}/data/raw/hesa/{out_name}.txt", skiprows=skip
        )
        ...
        out = pd.read_csv(
            f"{project_dir}/data/raw/hesa/{out_name}.txt",
            skiprows=skip,
            dtype={"UKPRN": str},
        )
...
def university_indicator(
    ...
    df = pd.merge(
        data, geo_data, left_on=["year_regions", "ukprn"], right_on=[year_col, "ukprn"]
    )
    df = df.groupby(["academic_year", region_id_col, groupby_year_col], as_index=False)[
        value
    ].sum()
...
def make_student_table(url):
    ...
    if "students" in os.listdir(f"{project_dir}/data/raw/hesa/"):
        ...
        out_files = [
            zipfile.ZipFile(io.BytesIO(rs.content)).extract(
                f"table-13-({year}).csv", f"{project_dir}/data/raw/hesa/students/"
            )
            for year in years
        ]
        ...
        graduates_all_years = pd.concat(
            [
                pd.read_csv(out_files[n], skiprows=14, dtype={"UKPRN": str})
                for n in np.arange(len(out_files))
            ],
            axis=0,
        )
        ...
        graduates_all_years["academic_year"] = [
            parse_academic_year(x) for x in graduates_all_years["academic_year"]
        ]

        graduates_all_years.to_csv(
            f"{project_dir}/data/raw/hesa/students.csv", index=False
        )

        print("parsed collected and parsed students")


def make_nuts_estimate(
    data,
    nuts_lookup,
    counter,
    name,
    ...
    if method == "time_consistent":
        d["nuts_code"] = [
            nuts_lookup[row[my_id]][get_nuts_category(row[year_var])]
            if row[my_id] in nuts_lookup.keys()
            else np.nan
        ...
        d["nuts_code"] = [
            nuts_lookup[row[my_id]]["nuts2_2016"]
            if row[my_id] in nuts_lookup.keys()
            else np.nan
            for rid, row in d.iterrows()
        ]
...
def calculate_perf(
    ...
    t_filt = multiple_nuts_estimates(
        filter_data(t, p_filter),
        nuts_lookup,
        set(table["category_marker"]),
        "category_marker",
        "value",
        year_var="academic_year",
        method=method,
    )
    ...
    if norm == True:
        ...
        u_filt = multiple_nuts_estimates(
            filter_data(t, unit_filter),
            nuts_lookup,
            set(table["category_marker"]),
            "category_marker",
            "value",
