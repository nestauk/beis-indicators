...
for nuts_level in [2, 3]:

#     import pdb; pdb.set_trace()

#Read the uni-nuts lookup
    with open(f'{project_dir}/data/interim/uni_nuts{nuts_level}_geos.json','r') as infile:
        uni_nuts = json.load(infile)

    nuts_academics = make_nuts_estimate(res_filtered, uni_nuts, 'number', 
            'academic_staff', 'academic_year')
...
#             'Research student FTE', 
    ...
    nuts_spaces = multiple_nuts_estimates(spaces, uni_nuts, space_vars, 
            'category', 'value', year_var='academic_year')
...
#Number of STEM graduates
    ...
    nuts_disciplines = multiple_nuts_estimates(grad_filtered, uni_nuts, disciplines,
            'subject_of_study', 'number', year_var='academic_year')
...
#We will filter the data to focus on full time postgraduate researchers
    ...
    nuts_postgrads = make_nuts_estimate(post_grad_filtered, uni_nuts, 'number',
            'postgrad_research', year_var='academic_year')
#STEM postgraduates
    post_grad_filter = {
...
#Extract information
    nuts_postgrad_discipline = multiple_nuts_estimates(post_grad_filtered, uni_nuts,disciplines,
            'subject_of_study', 'number', year_var='academic_year')
...
################
    ...
    hesa_dfs = [nuts_spaces, nuts_spaces, stem_students, stem_postgraduates, 
...
#             {'Grounds area': 'area_university_site'},
#             {'Number of buildings': 'total_university_buildings'}]

    for df, name_lookup in zip(hesa_dfs, name_lookups):
        ind = make_indicator(df, name_lookup, 'academic_year')
        fname =  list(name_lookup.values())[0] + f'.nuts{nuts_level}'
