...
def save_nuts_table(year, aux_path=f"{PROJECT_DIR}/data/aux/"):
    ...
    if not os.path.exists(f"{aux_path}{file}"):
        nuts = requests.get(
...
if __name__ == "__main__":
    ...
    # collect the trademark data from Eurostat
    trades = es.get_data_df("ipr_ta_reg")

    nuts_level = 2
    level = "nuts2"
    name = "eu_trademark_applications"
    ...
    sel = select_data(trades, list(range(2006, 2025)))
    # make indicator
    ind = make_indicator(
        sel,
        {"value": name},
