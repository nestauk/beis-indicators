<script>
	...
	const query_types = ['POST', ' ElasticSearch', 'Neo4j', 'MySQL'];
	const size = 12;
...
<div class='modal' on:click>
	...
	>
		...
		{#if date}
		<p><b>Data downloaded in date:</b> {date}</p>
		{/if}
		<p>
			<b>Available years:</b> {year_range[0]}
		...
		{#if endpoint_url}
			{#if api_type === 'FETCH'}
				{#if isObject(endpoint_url)}
					...
					<ul>
					{#each _.pairs(endpoint_url) as [text, href]}
						<li><span><ExternalLink {href} {text}/></span></li>
					{/each}
					</ul>
		...
		{#if !_.isUndefined(is_public) && !is_public}
			<p>
				<b>Access:</b>
				<span>This dataset is not public. </span>
				{#if auth_provider}
